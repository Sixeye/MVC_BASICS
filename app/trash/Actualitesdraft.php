<?php
    ...
    */
        ...
        static function createActualites()
        {
            ...
            {
                $post_title       = Utility::str_secur($_POST['ntitle']);
                $post_sentence    = Utility::str_secur($_POST['nsentence']);
                $post_content     = Utility::$_POST['content'];
                $post_author_id   = Utility::str_secur($_POST['nauthor_id']);
                $post_category_id = Utility::str_secur($_POST['ncategory_id']);
                $date = (now);

                $reqActualites = $conn->prepare('INSERT INTO actualites(title, sentence, content, date, author_id, category_id) VALUES(?, ?, ?, ?, ?, ?)');
    ...
    */
        ...
        static function fillActualites()
        {
            ...
            if (isset($_POST['nupdate']))
            {
                ...
                $reqActualites = $conn->prepare("SELECT * FROM actualites WHERE id = $id");
                $reqActualites->execute();
    ...
    */
        ...
        static function updateActualites()
        {
                ...
                {
                $nupdated_id = (int)(Utility::str_secur($_POST['nu_id']));
                $nupdated_title = Utility::str_secur($_POST['nu_title']);
                $nupdated_sentence = Utility::str_secur($_POST['nu_sentence']);
                $nupdated_content = $_POST['nu_content'];
                $nupdated_author_id = (int)(Utility::str_secur($_POST['nu_author_id']));
                $nupdated_category_id = (int)(Utility::str_secur($_POST['nu_category_id']));
                $date = (now);

                $udActualites = $conn->prepare('UPDATE actualites set title = ?, sentence = ?, content = ?, date = ?, author_id = ?, category_id = ? WHERE id = ?');
